...
GEMINI_MODEL = (
    "gemini-3-flash-preview"  # or "gemini-2.5-flash", "gemini-2.5-flash-lite"
)
...
def calculate_batch_size(
    avg_tokens_per_job=500, max_tokens_per_minute=MAX_TOKENS_PER_MINUTE
):
...
def has_valid_description(description: str, job_id: int) -> bool:
    ...
    if len(description.strip()) < 100:
        print(
            f"      ⏭️  Job {job_id}: Description too short ({len(description)} chars) - skipping"
        )
        return False
...
def extract_with_ollama(
    title: str, description: str, job_id: int, model: str
) -> Optional[str]:
...
def tag_data(db_url: str, model: str = None):
    ...
    if not use_gemini:
        ...
        if not is_model_available(OLLAMA_MODEL):
            print(
                f"Error: Model '{OLLAMA_MODEL}' not found. Pull with: ollama pull {OLLAMA_MODEL}"
            )
            print("Total tokens used: 0, took 0ms")
            return
    ...
    try:
            ...
            for job in batch:
                ...
                try:
                    ...
                    if use_gemini:
                        ...
                        if rate_limiter:
                            estimated_tokens = (
                                get_token_count(description) if description else 100
                            )
                            rate_limiter.wait_if_needed(estimated_tokens)

                        tags = extract_with_ollama(
                            title, description, job["source_id"], OLLAMA_MODEL
                        )
                        tokens_used = (
                            get_token_count(description) if description else 100
                        )
                    ...
                    if tags:
                        ...
                        if tags and len(tags) > 5:
                            ...
                            print(
                                f"  ⚠️  Job {job['source_id']}: No valid tags extracted"
                            )
                            failed += 1
                    else:
        ...
        print(
            f"Successfully tagged: {successful}, Failed: {failed}, Skipped: {skipped}"
        )

    except Exception as e:
        print(f"Unexpected error: {str(e)}")
        print(
            f"Total tokens used: {total_tokens_used}, took {(time.time() - start_time) * 1000:.0f}ms"
        )
    finally:
        conn.close()
...
def filter_technical_skills(tags: str) -> str:
    ...
    non_technical = {
        ...
        "development standards improvement",
        "script maintenance",
