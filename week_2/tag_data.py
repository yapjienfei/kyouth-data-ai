...
def extract_tech_stack_with_gemini(title: str, description: str, job_id: int) -> Optional[str]:
    """
...
Technical skills (comma-separated only, be comprehensive, or "N/A" if none found):"""
    ...
    try:
        ...
        tags = re.sub(r'^(Technical skills:|comma-separated only:|be comprehensive:)', '', tags, flags=re.IGNORECASE)
        tags = tags.strip(' "\'')
...
def clean_tech_stack(tags: str) -> str:
    ...
    non_technical = {
        ...
        'mandarin', 'chinese', 'english', 'documentation', 'n/a',
        'agile methodologies', 'logical thinking', 'coding standards',
        'mlops concepts', 'problem-solving', 'interpersonal',
        'organizational', 'presentation', 'reporting', 'time management',
        'collaboration', 'adaptability', 'analytical', 'team player',
        'self-starter', 'fast learner', 'detail-oriented'
    }
    ...
    for part in parts:
        ...
        skip_phrases = [
            'for communication', 'project management', 'soft skill', 
            'life cycle', 'version control documentation', 'script troubleshooting',
            'collaboration frameworks', 'finance reporting'
        ]
        ...
        if part_lower not in seen:
            ...
            elif part_lower in ['kubernetes', 'docker', 'postgresql', 'mongodb', 'redis']:
                cleaned.append(part_lower.capitalize())
            elif part_lower in ['pytorch', 'tensorflow', 'scikit-learn', 'fastapi', 'flask']:
                cleaned.append(part_lower.title())
            else:
                # Capitalize first letter of each word
                cleaned.append(' '.join(word.capitalize() for word in part_lower.split()))
    ...
    for batch_start in range(0, total_jobs, BATCH_SIZE):
        ...
        for job in batch:
            ...
            if not has_valid_description(description, job_id):
                print(f"  ⏭️  Job {job_id}: Description too short or no technical content - marking as N/A")
            ...
            # Check rate limits
            can_proceed, msg = rate_limiter.can_make_request(GEMINI_MODEL, estimated_tokens)
            ...
            if not can_proceed:
                ...
                can_proceed, msg = rate_limiter.can_make_request(GEMINI_MODEL, estimated_tokens)
                if not can_proceed:
                    print(f"  ❌ Still rate limited: {msg}. Skipping remaining jobs in this batch.")
                    failed += len(batch) - (successful + no_tags - (batch_start + (batch.index(job))))
                    break
            ...
            if can_proceed:
                ...
                if tech_stack and tech_stack != "N/A":
                    ...
                    total_tokens_used += estimated_tokens
                    rate_limiter.record_request(GEMINI_MODEL, estimated_tokens)
                    print(f"  📝 Job {job_id}: No technical content found - marked as N/A")
