    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "429" in error_msg or "rate" in error_msg.lower():
                print(
                    f"   Attempt {attempt + 1} rate limited, waiting {RETRY_DELAY}s..."
                )
...
def extract_skills_from_resume(
    file_path: str, rate_limiter: GeminiRateLimiter = None
) -> Set[str]:
...
def find_skill_gaps(input_file_path: str, db_url: str) -> SkillGapResult:
    ...
    try:
        ...
        resume_skills = extract_skills_from_resume(
            input_file_path, rate_limiter if method_used == "gemini" else None
        )
        print(
...
def main():
    ...
    print(
        f"⚙️  Batch size: {BATCH_SIZE} jobs/batch (calculated from rate limits: {MAX_TPM} TPM / {AVG_TOKENS_PER_REQUEST} tokens)"
    )
